...
// --- CUSTOMIZATION ---
// Points to your specific video asset.
...
export default function App() {
  ...
  const InvitationView = () => (
    <div className="min-h-screen bg-[#fff5f6] flex flex-col items-center py-12 px-4 font-sans text-stone-800">
      {dbError && (
        <div className="max-w-xl w-full mb-6 p-4 bg-rose-50 border border-rose-100 rounded-xl flex gap-3 text-rose-700 shadow-sm">
          <ShieldAlert className="shrink-0" size={20} />
          <div className="text-xs font-bold">{dbError}</div>
        </div>
      )}

      <div className="max-w-xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden border border-rose-100">
        {/* Video Header - Pastel Fallback */}
            ...
            <div className="space-y-10">
              ...
              <form onSubmit={handleRSVPSubmit} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input required name="name" className="w-full px-6 py-4 bg-rose-50/50 border border-rose-100 rounded-2xl outline-none focus:ring-2 focus:ring-rose-200 transition-all text-sm placeholder:text-rose-200" placeholder="Full Name" />
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-rose-800 ml-1">Additional Guests</label>
                  <select name="guests" className="w-full px-6 py-4 bg-rose-50/50 border border-rose-100 rounded-2xl outline-none text-sm appearance-none cursor-pointer">
                ...
                <div className="space-y-2">
                  ...
                  <textarea name="dietary" rows="3" className="w-full px-6 py-4 bg-rose-50/50 border border-rose-100 rounded-2xl outline-none text-sm resize-none placeholder:text-rose-200" placeholder="Allergies or song requests?"></textarea>
                </div>
      ...
      <button onClick={() => setView('login')} className="mt-12 text-rose-300 hover:text-rose-500 flex items-center gap-1 text-[10px] uppercase tracking-widest transition-colors font-bold"><Settings size={12} /> Admin</button>
    </div>
  );
      ...
      <div className="min-h-screen bg-[#fff5f6] p-4 md:p-8 font-sans">
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-center mb-10">
            ...
            <button onClick={() => {
              const headers = ["Name", "Attending", "Guests", "Notes", "Date"];
              const rows = responses.map(r => [`"${r.name}"`, r.attending ? "Yes" : "No", r.guests, `"${r.dietary || ''}"`, `"${r.timestamp}"`]);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10 text-center">
            <div className="bg-white p-10 rounded-[2.5rem] border border-rose-100 shadow-sm">
              <p className="text-rose-300 text-[10px] font-black uppercase tracking-[0.2em] mb-2">Headcount</p>
              <p className="text-4xl font-black text-rose-900">{totalAttending + totalGuests}</p>
            </div>
            <div className="bg-white p-10 rounded-[2.5rem] border border-rose-100 shadow-sm">
              <p className="text-rose-300 text-[10px] font-black uppercase tracking-[0.2em] mb-2">Accepts</p>
              <p className="text-4xl font-black text-rose-600">{totalAttending}</p>
            </div>
            <div className="bg-white p-10 rounded-[2.5rem] border border-rose-100 shadow-sm">
              <p className="text-rose-300 text-[10px] font-black uppercase tracking-[0.2em] mb-2">Declines</p>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-xl border border-rose-100 overflow-hidden">
            <table className="w-full text-left text-xs">
              <thead className="bg-rose-50/50 border-b border-rose-100 text-rose-800 uppercase text-[9px] font-bold tracking-widest">
                <tr><th className="px-8 py-6">Guest</th><th className="px-8 py-6">Status</th><th className="px-8 py-6">Plus</th><th className="px-8 py-6">Notes</th></tr>
              </thead>
              <tbody className="divide-y divide-rose-50 text-stone-700">
                {responses.sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp)).map((resp) => (
                  <tr key={resp.id} className="hover:bg-rose-50/30 transition-colors">
                    <td className="px-8 py-6 font-bold text-rose-900">{resp.name}</td>
                    ...
                    <td className="px-8 py-6 font-bold">{resp.guests}</td>
